...
export async function captureBeforeSnapshot(changeType: ScheduledChangeType, targetId: number | null): Promise<Prisma.InputJsonValue> {
  switch (changeType) {
    ...
    case "ROLE_PERMISSIONS": {
      ...
      return { roleName: role.name, permissions: role.permissions.map((p) => ({ module: p.module, canView: p.canView, canCreate: p.canCreate, canEdit: p.canEdit, canDelete: p.canDelete, canExport: p.canExport, canImport: p.canImport, canDuplicate: p.canDuplicate })) };
    }
  }
}
