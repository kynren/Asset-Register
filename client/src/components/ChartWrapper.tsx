...
export function SimpleBarChart({ data, xKey, yKey, height = 260 }: { data: any[]; xKey: string; yKey: string; height?: number }) {
  return (
    <ResponsiveContainer width="100%" height={height}>
      <BarChart data={data} margin={{ left: -20 }}>
        ...
        <Tooltip />
        <Bar dataKey={yKey} fill="#1d4ed8" radius={[4, 4, 0, 0]} />
      </BarChart>
    </ResponsiveContainer>
  );
}
...
export function SimplePieChart({ data, dataKey, nameKey, height = 260 }: { data: any[]; dataKey: string; nameKey: string; height?: number }) {
  return (
    <ResponsiveContainer width="100%" height={height}>
      <PieChart>
        <Tooltip />
        <Pie data={data} dataKey={dataKey} nameKey={nameKey} outerRadius={90} label={(entry) => entry[nameKey]}>
          {data.map((_, i) => (
