...
export function CollectionsManageModal({ onClose }: { onClose: () => void }) {
  const queryClient = useQueryClient();
  const [newName, setNewName] = useState("");
  const [newDescription, setNewDescription] = useState("");
  const [editingId, setEditingId] = useState<number | null>(null);
  const [editName, setEditName] = useState("");
  const [deleteTarget, setDeleteTarget] = useState<DocCollectionItem | null>(null);
  ...
  const createMutation = useMutation({
    mutationFn: () => axiosClient.post("/docs/collections", { name: newName, description: newDescription || undefined }),
  ...
  const renameMutation = useMutation({
    mutationFn: (id: number) => axiosClient.patch(`/docs/collections/${id}`, { name: editName }),
    onSuccess: () => { setEditingId(null); invalidate(); },
    onError: (err: any) => setError(err.response?.data?.error || "Could not rename collection."),
  });

  const deleteMutation = useMutation({
    mutationFn: (id: number) => axiosClient.delete(`/docs/collections/${id}`),
      ...
      <PermissionGate module="docs" action="create">
        <div className="row gap-2 flex-wrap" style={{ marginBottom: 16, alignItems: "flex-end" }}>
          <div className="field" style={{ marginBottom: 0, flex: "1 1 160px" }}>
            ...
            <input className="input" value={newName} onChange={(e) => setNewName(e.target.value)} placeholder="e.g. Asset Register SOPs" />
          </div>
          <div className="field" style={{ marginBottom: 0, flex: "1 1 160px" }}>
          ...
          >
            {editingId === c.id ? (
              ...
              <div>
                ...
                <div className="muted" style={{ fontSize: 11 }}>{c.documentCount} {c.documentCount === 1 ? "document" : "documents"}</div>
              </div>
            )}
              ...
              <div className="row gap-1">
                ...
                <PermissionGate module="docs" action="delete">
                  <button
                    ...
                    title={c.documentCount > 0 ? "Move or delete its documents first" : "Delete collection"}
