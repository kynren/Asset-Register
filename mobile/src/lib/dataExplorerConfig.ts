...
// Mirrors client/src/pages/dashboard/dataExplorerConfig.ts (itself mirroring
// server/src/modules/dashboard/dataExplorer.ts's SOURCES registry) so the mobile Report Builder
// drives the same /reports/preview + /reports POST/PATCH endpoints with the same shape. The server
// independently re-validates every field/operator combination it receives.
